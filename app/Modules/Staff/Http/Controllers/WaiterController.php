...
class WaiterController extends Controller
{
    ...
    private function compileDailyReportData($staffId)
    {
    ...
    public function printDailyReport()
    {
        try {
            ...
            return view('waiter.print-daily-report', compact('report', 'settings'));
        } catch (\Exception $e) {
    ...
    public function pdfDailyReport()
    {
        try {
            ...
            $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('waiter.print-daily-report', compact('report', 'settings'));
            
            $filename = 'raport-zilnic-' . Str::slug(session('staff_name')) . '-' . now()->format('d-m-Y') . '.pdf';
