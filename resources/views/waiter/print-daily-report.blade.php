...
<head>
    ...
    <title>Raport Zilnic Ospătar - {{ $report['staff_name'] }}</title>
    <style>
        @page {
            size: 80mm auto;
            margin: 0;
...
<body onload="window.print()">
    ...
    <div class="no-print">
        ...
        <a href="{{ route('waiter.pdf-daily-report') }}" class="btn btn-secondary">DESCARCĂ PDF</a>
        <p style="font-size: 11px; margin-top: 6px; color: #64748b;">Se va deschide automat fereastra de printare.</p>
    </div>

    <div class="text-center">
        <div class="bold xlarge">{{ $settings->site_name ?? 'Daser Restaurant OS' }}</div>
        <div>{{ $settings->address ?? '' }}</div>
        <div>CIF: {{ $settings->company_vat_id ?? '' }}</div>
        <div class="border-double"></div>
        <div class="bold large">RAPORT ZILNIC OSPĂTAR</div>
    ...
    <div class="border-b"></div>
    <div class="bold">SUMAR VÂNZĂRI</div>
    <div class="border-b"></div>
    ...
    <table>
        ...
        <tr>
            ...
            <td class="text-right bold">{{ $report['paid_orders_count'] }}</td>
        </tr>
        <tr class="bold">
    ...
    <div class="border-b"></div>
    <div class="bold">PRODUSE VÂNDUTE</div>
    <div class="border-b"></div>
    ...
    <table>
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="3" class="text-center" style="padding: 10px 0;">Niciun produs vândut în această tură.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div style="margin-top: 25px; border-top: 1px dashed #000; padding-top: 8px; text-align: center; color: #000; font-size: 8px;">
        Powered by Daser Restaurant OS
    </div>
